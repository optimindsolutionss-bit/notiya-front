// Frontend/src/pages/Pantalla/PromoRibbon.jsx
...
function TarjetaPromo({ promo, productos, destacar, paleta }) {
  ...
  return (
    <Box
      ...
      animate={
        destacar
          ? { boxShadow: [`0 0 0 1px ${paleta.accentMuted}`, `0 0 0 1px ${paleta.accent}`, `0 0 0 1px ${paleta.accentMuted}`] }
          : undefined
      }
      ...
      sx={{
        ...
        border: `1px solid ${paleta.accentMuted}`,
        flexShrink: 0,
        minWidth: 260,
      }}
    >
      ...
      <Box sx={{ minWidth: 0 }}>
        ...
        <Box sx={{ display: "flex", alignItems: "baseline", gap: 1 }}>
          <Typography sx={{ fontFamily: fontMono, color: paleta.accent, fontWeight: 700, fontSize: "1.1rem" }}>
...
export default function PromoRibbon({ promociones, productos }) {
  ...
  return (
    <Box sx={{ overflow: "hidden", px: { xs: 3, md: 6 }, py: 1.5 }}>
      ...
      >
        {items.map((promo, i) => (
          <TarjetaPromo key={`${promo.id}-${i}`} promo={promo} productos={productos} destacar={destacar} paleta={paleta} />
        ))}
      </Box>
    </Box>
  );
